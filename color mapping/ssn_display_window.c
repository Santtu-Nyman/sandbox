...
{
	// down scaling looks horrible. also this is very slow.
	// how do i scale stuff without it looking so bad?
...
{
	const size_t allocation_quantum = 64;

	HANDLE heap = GetProcessHeap();
	if (!heap)
		return ERROR_OUTOFMEMORY;
	display_window_instance_t* instance = (display_window_instance_t*)HeapAlloc(heap, 0, ((sizeof(display_window_instance_t) + (allocation_quantum - 1)) & ~(allocation_quantum - 1)) + ((((size_t)image_height * (size_t)image_width * sizeof(DWORD)) + (allocation_quantum - 1)) & ~(allocation_quantum - 1)));
	if (!instance)
		return ERROR_OUTOFMEMORY;
	...
	instance->image_pixels = (DWORD*)((UINT_PTR)instance + ((sizeof(display_window_instance_t) + (allocation_quantum - 1)) & ~(allocation_quantum - 1)));
	for (size_t pixel_count = (size_t)image_height * (size_t)image_width, pixel_i = 0; pixel_i != pixel_count; ++pixel_i)
		instance->image_pixels[pixel_i] = image_pixels[pixel_i];
	...
	HWND window;
	DWORD error = ssn_create_window(WS_EX_APPWINDOW, &window_class, window_title, WS_OVERLAPPEDWINDOW | WS_VISIBLE, CW_USEDEFAULT, CW_USEDEFAULT, 640, 480, 0, 0, 0, instance, &window);
	if (error)
	{
	...
	while (instance->running && GetMessageW(&instance->message, instance->window_handle, 0, 0) == TRUE)
	{
