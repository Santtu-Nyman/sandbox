...
int main(int argc, char** argv)
{
	...
	for (size_t command_index = 0; command_index != command_count; ++command_index)
	{
		...
		if (!get_command_by_index(input, command_index, &command_type, &command_data, &next_command))
		{
			if (!(argc > 2))
				free(ouput_file_name);
			free(input);
...
bool get_command_count(const char* commands, size_t* count)
{
	...
	size_t i = 0;
	for (const char* next_command_line = commands; next_command_line;)
		if (get_command_by_index(commands, i, &ignored_type, &ignored_data, &next_command_line))
			++i;
		else
			return false;
	*count = i;
